...
class Razones_Sociales_Model extends ANT_Model {

	protected static $table = 'razones_sociales';
	...
	static function get_grid_info($where = NULL, $list = NULL, $agent = NULL) {
		...
		$result = Razones_Sociales_Model::Load($options);
		return $result;
	}
	static function get_select($where = NULL, $list = NULL, $agent = NULL) {
		...
		foreach ($result as $key ) {
			$lista .= '<option value="'.$key['id'].'">'.$key['name'].'</option>';
		}
		return $lista;
	}
