...
class Puestos extends ANT_Controller
{
	...
	function add()
	{
		...
		$this->_load_views('Puestos/add', $data);
	}
	function edit($id)
	{
	...
	function get_info_Puestos()
	{
		$post = $this->input->post();
		$data = Puestos_Model::Load(array(
			'select' => "*",
			'where' => 'id=' . $post['id'],
			'result' => '1row'
		));
	...
	{
		$aux = Puestos_Model::get_grid_info();
	...
	function save_info()
	{
		...
		if ($post['id'] == 0) {
			$result = Puestos_Model::Insert($post);
		} else {

			$result = Puestos_Model::Update($post, 'id=' . $post['id']);
		}
