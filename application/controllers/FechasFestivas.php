...
class FechasFestivas extends ANT_Controller
{
	...
	function index()
	{
		$data['title'] = 'FechasFestivas';
		$data['view'] = 'grids/FechasFestivas';
		$data['styles'] = 'jquery.shuttle';
		$data['js_scripts'] = 'lib/jquery.shuttle';
		$data['user_id'] = $this->tank_auth->get_user_id();
		$this->_load_views('FechasFestivas/list', $data);
	}
	function add()
	{
		$data['title'] = 'Agregar Vehiculo';
		$data['view'] = 'forms/FechasFestivas';
		$data['id'] = 0;
		$data['styles'] = 'jquery.shuttle';
		$data['js_scripts'] = 'lib/jquery.shuttle';
		...
		$this->_load_views('FechasFestivas/add', $data);
	}
	function edit($id)
	{
	...
	function get_info_FechasFestivas()
	{
		$post = $this->input->post();
		$data = FechasFestivas_Model::Load(array(
			'select' => "*",
			'where' => 'id=' . $post['id'],
			'result' => '1row'
		));
	...
	function get_FechasFestivas()
	{
		$aux = FechasFestivas_Model::get_grid_info();
		$data['head'] = "<tr><th>Fecha</th>
	...
	function save_info()
	{
		...
		if ($post['id'] == 0) {
			$result = FechasFestivas_Model::Insert($post);
		} else {

			$result = FechasFestivas_Model::Update($post, 'id=' . $post['id']);
		}
	...
	function eliminar()
	{
		...
		FechasFestivas_Model::Delete('id=' . $id);
	}
}
