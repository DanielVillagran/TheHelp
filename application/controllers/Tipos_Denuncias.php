...
class Tipos_Denuncias extends ANT_Controller {
	...
	function index() {
		$data['title'] = 'Tipos de Denuncias';
		$data['view'] = 'grids/Tipos_Denuncias';
		$data['styles'] = 'jquery.shuttle';
		$data['js_scripts'] = 'lib/jquery.shuttle';
		$data['user_id'] = $this->tank_auth->get_user_id();
		$this->_load_views('Tipos_Denuncias/list', $data);
	}
	function add() {
		$data['title'] = 'Agregar Tipo de denuncia';
		$data['view'] = 'forms/Tipos_Denuncias';
		$data['id'] = 0;
		$data['styles'] = 'jquery.shuttle';
		$data['js_scripts'] = 'lib/jquery.shuttle';
		$data['user_id'] = $this->tank_auth->get_user_id();
		$this->_load_views('Tipos_Denuncias/add', $data);
	}
	function edit($id) {
	...
	function get_info_Tipos_Denuncias(){
		$post = $this->input->post();
		$data = Tipos_Denuncias_Model::Load(array('select' => "*",
				'where' => 'id=' . $post['id'],
				'result' => '1row'));
				$this->output_json($data);
	}
	function get_Tipos_Denuncias() {
		$aux = Tipos_Denuncias_Model::get_grid_info();
	...
	function save_info() {
		...
		if ($post['id']==0) {
			$result = Tipos_Denuncias_Model::Insert($post);
		} else {
		
			$result = Tipos_Denuncias_Model::Update($post, 'id=' . $post['id']);
		}
		$this->output_json($result);
	}
	function eliminar(){
		$id = $this->input->post("id");
		Tipos_Denuncias_Model::Delete('id='.$id);
	}
}
