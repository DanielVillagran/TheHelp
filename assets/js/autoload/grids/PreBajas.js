...
function grid_load_data() {
    $.ajax({
        url: "/Colaboradores/get_Colaboradores/prebaja",
        type: 'POST',
        data: {
...
$('#groups_grid').footable().on('click', '.row-edit', function (e) {
    ...
    window.location.href = '/PreBajas/edit/' + idemp;
});
$('#groups_grid').footable().on('click', '.row-delete', function (e) {
    e.preventDefault();
    var idemp = $(this).attr('rel');
    swal({
        title: "<p id='pswalerror'>Estas seguro que deseas reactivar este elemento?</p>",
        html: "<p id='psswalerror'>Estas seguro que deseas reactivar este elemento?</p>",
        type: "warning",
