...
$(document).ready(function () {
    ...
    $("#sidebar").load("../components/sidebar-extra.html");
    $.ajax({
...
$(function () {
    $('.btn-switch-noti').change(function () {
        if ($(this).is(':checked')) {
            ...
            $('.dropdown-select.select-colonia').attr('style', 'pointer-events: all;');
            $('.row-colonia').css('opacity', '1');
        } else {
            $('.dropdown-select.select-colonia').attr('style', 'pointer-events: none;');
...
$('.btn-participar-modal').click(function () {
    if ($(".input-participar").val() == "") {
        $(".input-participar").tooltip("show");
    } else {
        $("#modalTelefono").modal("hide");
        $(".btn-participar").hide();
        $(".p-participar").fadeIn();
        $(".input-participar").tooltip("hide");
    }
...
$(".row-si-no-1 .btn").click(function () {
    if ($(this).hasClass("btn-lg-si")) {
        $(".row-si-no-1 .btn-lg-no img").attr("src", "/images/icons/icon-close-black.svg");
        $(".row-si-no-1 .btn-lg-no").removeClass("active-no");
        $(this).addClass("active-si");
        $(this).find("img").attr("src", "/images/icons/icon-check-si.svg");
    } else if ($(this).hasClass("btn-lg-no")) {
        $(".row-si-no-1 .btn-lg-si img").attr("src", "/images/icons/icon-check-si-black.svg");
...
$(".row-si-no-2 .btn").click(function () {
    ...
    if ($(this).hasClass("btn-lg-si")) {
        $(".row-si-no-2 .btn-lg-no img").attr("src", "/images/icons/icon-close-black.svg");
        $(".row-si-no-2 .btn-lg-no").removeClass("active-no");
        $(this).addClass("active-si");
        $(this).find("img").attr("src", "/images/icons/icon-check-si.svg");
    } else if ($(this).hasClass("btn-lg-no")) {
        $(".row-si-no-2 .btn-lg-si img").attr("src", "/images/icons/icon-check-si-black.svg");
...
$(".row-si-no-3 .btn").click(function () {
    ...
    if ($(this).hasClass("btn-lg-si")) {
        $(".row-si-no-3 .btn-lg-no img").attr("src", "/images/icons/icon-close-black.svg");
        $(".row-si-no-3 .btn-lg-no").removeClass("active-no");
        $(this).addClass("active-si");
        $(this).find("img").attr("src", "/images/icons/icon-check-si.svg");
    } else if ($(this).hasClass("btn-lg-no")) {
        $(".row-si-no-3 .btn-lg-si img").attr("src", "/images/icons/icon-check-si-black.svg");
...
$('.btn-enviar-denuncia').click(function () {
    $(".d-loader-denuncia").css("visibility", "visible");
});

/*  BTN ENVIAR DENUNCIA */


/*  BTN ACTIVE METODO DE PAGO */
